...
TalentMatch requires two AI agent layers: a structured CV analysis agent that extracts typed data from candidate CVs, and a conversational assistant that answers HR questions using real database tools with persisted conversation memory. This capability defines the agent and tool structure required to support both layers.
