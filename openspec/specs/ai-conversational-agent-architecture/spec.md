...
Define the internal tool contract, authorization pattern, memory middleware pipeline, and agent lifecycle for the conversational agent architecture in TalentMatch.
...
Conversation persistence SHALL be handled by the `RememberConversation` middleware auto-registered by the SDK when the agent uses the `RemembersConversations` trait and has a conversation participant. The middleware SHALL check `$agent->currentConversation()` to decide whether to create a new conversation or continue an existing one.
