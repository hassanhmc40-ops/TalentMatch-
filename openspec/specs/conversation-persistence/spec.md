...
Define the data persistence model for SDK-managed agent conversations, bridging the gap between the raw `agent_conversations` / `agent_conversation_messages` tables and the TalentMatch domain model via Eloquent models, foreign key relationships, and indexes.
...
After a conversation is created via `ConversationalAgent::prompt()`, the `ConversationController::store()` method SHALL persist the `candidate_analysis_id` on the `agent_conversations` row.
