...
Define the prompt strategy for structured CV analysis, including system prompt structure, score calculation reasoning, anti-hallucination guardrails, and few-shot examples. This capability covers the design of `CvAnalysisAgent::instructions()` and the few-shot context in `AnalyseCvJob`.
